...
An alternative view of the dual parameters is that they parameterize approximate likelihoods, as shown in \citep{khan2017conjugate, adam2021dual}. Then, we can rewrite the {\color{red} sparse} GP posterior for $q(\vu)$ in terms of the following approximate normal distributions:

\begin{equation}
\begin{aligned}
 q(\vu) 
 	&\propto \Norm(0, \MKzz) \prod_{i=1}^n \exp \! \left(-\frac{\hat{\beta}_i}{2}(\tilde{y}_i - \va_i^\top \vu)^2 \right) \\
 	&= \Norm(0, \MKzz) \, \exp\left((\bar{\vy} - \vu)^\top \bar{\MSigma}^{-1}(\bar{\vy} - \vu)\right)  
\end{aligned}
\end{equation}

% The first normals are the approximate likelihoods 
where $\va_i^\top = \vkzi^\top \MKzz^{-1}$ and $\tilde{y}_i = \hat{\alpha}_i + f_{\vw^*}(\vx_i)\hat{\beta}$,  replacing $\va_i^\top \vu$ with $\vx_i^{\top}\vw$ and we can see the similarity between the sparse GP and a linear model where the kernel matrix determines the prior. To arrive at the MVN Gaussian, we need to rearrange the quadratic terms to match the prior sufficient statistics. Performing conjugate summation of prior and likelihood, we would arrive at the sparse GP posterior at the inducing points. The correlated covariance structure $\MSigma$ and the corresponding $\tilde{\vy}$ are simply a different form of the sparse dual variables,
\begin{equation}
\quad \bar{\MB}^{-1} =  \MKzz \MB_\vu^{-1} \MKzz \quad \bar{\vy} = \MKzz \MB_{\vu}\valpha_{\vu}.
\end{equation}
