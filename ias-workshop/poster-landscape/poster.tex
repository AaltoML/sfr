...
\begin{minipage}{\textwidth}
  ...
  % \end{tikzpicture}
    ...
    %  \newcommand{\myline}[1]{\protect\tikz[baseline=-.5ex,line width=1.6pt]\protect\draw[draw=#1](0,0)--(1.2em,0);}
    \caption{\footnotesize \textbf{\our \ (\myline{steelblue31119180}) requires fewer inducing points $M$} to converge to good NLPD than GP subset (\myline{darkorange25512714}) on UCI classification.}
    \end{subfigure}
    \end{figure}
...
% consequently the posterior) being only a locally linear approximation of the neural network model
  ...
  \begin{block}{Outlook}
   ...
   \begin{itemize}
    ...
    % \begin{itemize}
    %     % \item Calculating dual parameters is costly in memory and computation,
