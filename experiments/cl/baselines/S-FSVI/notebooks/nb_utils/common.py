...
def show_final_average_accuracy(exp, runner: str = "fsvi"):
    if runner == "fsvi":
        ...
        logging.info(f"Final Accuracy: {training_log_df[cols].iloc[-1].values.mean()}")
        ...
        if "training_log_df" in exp["chkpt"]:
            ...
            logging.info(f"Final Accuracy: {exp['chkpt']['result']['mean_accuracies'][0]}")
    elif runner == "vcl":
        print(exp['chkpt']['result'][-1].mean())
        logging.info(f"Final Accuracy: {exp['chkpt']['result'][-1].mean()}")
    else:
