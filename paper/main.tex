...
\label{app:method}
%
Key to our method is the ability to compute the dual parameters in \cref{eq:dual_param}. For general exponential family likelihoods we can derive simpler expressions. We now detail two common losses dual parameters the Gaussian (MSE) and the Bernoulli (cross entropy).  The general method is to rewrite the exponential family in the $\log$ of its natural form \ie:
\begin{equation}
 \ell(f_\weights(\mathbf{x}_{i}), y_i) = -\log p()
\end{equation}
