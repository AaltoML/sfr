...
\textbf{Dual Gaussian Process} A Gaussian process (GP) is a prior over functions $f \sim \GP(\mathbf{0}, \kappa(\cdot, \cdot))$. The prior is characterized by a covariance kernel function $\kappa(\vx,\vx')$, where $\vx, \vx'$ are input vectors in $\R^{\mathrm{D}}$. Once combined with a data set ${\mathcal{D} = \{(\vx_i, y_i)\}_{i=1}^N}$ with $N$ examples, each modelled by the likelihood $p(\vy_i \mid \vf_i)$ at function $\vf_i = f(\vx_i)$\, we can form a posterior proccess using multivariate conditioning. The drawbacks to GPs are their lack of scaling to large datasets and non-tractability with non-Gaussian likelihoods.
