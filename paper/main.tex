...
Although the true posterior $p(\vw \mid \dataset)$ is intractable given the non-linearities of the neural network, one can resort to sampling techniques such as Hamiltonian Monte-Carlo (HMC). However, even in this case, the high computational costs make them impractical meaning for most applications we need approximate inference techniques. In \cref{fig:banana}, we show a qualitative example of the induced function space posterior obtained with our method compared to the posterior obtained through HMC sampling on the Banana toy dataset. \todo{make it more straightforward that we are computationally cheaper? A: Not sure we are reallying trying to sell our method here}
