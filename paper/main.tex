...
The Hessian can be efficiently approximated using the generalised Gauss-Newton approximation \cite{botev2017practical} or by Kronecker factorisation \cite{martens2015optimizing,ritter2018kfac}.
...
Quantifying uncertainty in a Bayesian framework means finding a posterior of $p(\vw \mid \dataset)$. The posterior represents the uncertainty over our parameters based on the prior and the data. The posterior structure directly relates to the optimization loss around the MAP weights $\vw^*$. Furthermore, the landscape in the weight space induces functions produced by the neural network's deterministic mapping. If one was to sample from the weight posterior, the corresponding functions created can be viewed as perturbed versions of the $f_{\vw^*}$. In most applications, we care about predictions from the neural network, so the distribution is in the function space and not in the weight space. 

Although the true posterior is intractable given the non-linearities of the neural network, one can resort to sampling techniques such as Hamiltonian Monte-Carlo (HMC). However, even in this case, the high computational costs make them impractical meaning for most applications we need approximate inference techniques. In \cref{fig:banana}, we show a qualitative example of the induced function space posterior obtained with our method compared to the posterior obtained through HMC sampling on the Banana toy dataset. \todo{make it more straightforward that we are computationally cheaper?}
...
Following the work of \citep{khan2019approximate}, we can view this approximation as building an approximate linear model of the neural network as $\hat{f}_\weights(\mathbf{x}_i) = \Jac{\weights_*}{\mathbf{x}_i} \, \weights$, where $\Jac{\weights}{\mathbf{x}_i} \coloneqq \nabla_\weights f(\mathbf{x}_i)^\top \in \R^{C \times P}$ is the Jacobian at the MAP. Using the Hessian of the approximate model, we arrive at the Laplace-GGN posterior for $q(\vw)$.  

Recent work \citep{khan2019approximate,maddox2021fast} shows that linearizing approximations in the weight space lead to function space equivalent approximations. As our goal is to build a sparse functional representation of a neural network, a starting point is the non-sparse  $q(\vf)$ function space posterior approximation. We choose to use a Gaussian process, as is typical, an extension of the multivariate normal that characterizes a distribution over functions in terms of the first two moments, a mean $\mu(\cdot)$ and covariance  (or kernel) function $\kappa(\cdot,\cdot)$. Thus, we need these two functions based on the neural network to build the Gaussian process posterior. Gaussian distributions remain tractable under linear transformations. Therefore, a linear function in terms of parameters can be converted from the weight space to the function space (see Ch.~2.1 in \cite{rasmussen2006gaussian}) as follows:\begin{equation} \label{eq:weight_func}
...
\begin{enumerate}
  ...
  \item \textbf{Function-space updates} How faster are our function-space updates relative to retraining from scratch? Do they improve predictive performance? Are they as good as retraining from scratch?
