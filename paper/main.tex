...
% Methods
We compare our method against two categories of methods: {\em (i)}~weight-regularization methods: EWC, SI, and VCL (which additionally incorporates coresets), and {\em(ii)}~function-based regularization methods: DER, FROMP, S-FSVI, SFR. We also introduce an ablation study where we replace $\bar{\MB_t}^{-1}$ with an identity matrix $\MI_M$, equivalent to assuming a GP with i.i.d.\ prior with a unit variance (giving an L2 regularizer).
Following \cite{rudner2022continual}, we use 200 points per tasks. Additionally,  we consider only 40 points for S-MNIST, to investigate if our method was still capable of capture more information due to its sparse formulation.
