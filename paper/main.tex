...
\begin{table}[t!] 
  \centering\scriptsize
  \caption{Comparisons and ablations on UCI data with negative log predictive density (NLPD, lower better). Our sparse \our ($M=256$) is on par with or better than full models (left) and outperforms the GP subset approach of \cite{todo}. See \cref{app:uci} for additional tables with comparisons.} 
	\label{tbl:uci}
...
\label{sec:uci}
%
We consider the same 8 UCI~\cite{UCI} classification benchamrks tasks and setup as were considered in \cite{todo}. We use a two-layer MLP for all tasks and follow the setup in \cref{sec:methods} for \our. After training, we construct the \our dual and use the resulting model for uncertainty quantification. We performed hyperparameter search over the prior precision $\delta$, and ran the experiment over $10$ seeds. %Depending on task dimension and complexity, our model can match the neural network MAP in negative log-predictive density (NLPD) even with a low number of inducing points (see \cref{tbl:uci} left).
