...
class FrameStackWrapper(dm_env.Environment):
    ...
    def _transform_reward(self, time_step):
        ...
        r = (
            np.array(list(self._rewards)) * self._reward_weight
        ).sum()  # weigheted sum over stacking rewards
...
class ConcatObsWrapper(dm_env.Environment):
    ...
    def observation_spec(self):
        return dm_env.specs.Array(
            shape=self.obs_shape, dtype="float64", name="observation"
        )
    ...
    def _concate_obs(self, time_step):
        ...
        return time_step._replace(
            observation=np.concatenate([v.flatten() for v in obs.values()])
        )
...
def make_env(env_name: str, task_name: str, seed: int, action_repeat: int):
    """
