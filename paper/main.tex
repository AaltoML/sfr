...
where the $ij$th entry of the matrix $\MKzz \in \R^{m \times m}$ is $\kappa(\vz_i,\vz_j)$, $\vk_{\vz i}$\todo{the equation above uses $\vkzs$ not $\vk_{\vx i}$} denotes a vector where each $j$th element is $\kappa(\vz_j, \vx_i)$, and $\kappa_{ii} = \kappa(\vx_i, \vx_i)$.  The dual parameters consist of the vector $\valpha \in \R^{m}$ and the matrix $\vbeta \in \R^{m \times m}$, defined as
\begin{equation} 
