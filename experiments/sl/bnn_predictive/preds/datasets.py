...
from experiments.sl.bnn_predictive.preds import DATA_DIR
...
class SVHN(dset.SVHN):
    def __init__(
        self, root=DATA_DIR, train=True, download=True, transform=CIFAR10_transform
    ):
...
class UCIClassificationDatasets(data.Dataset):
    ...
    def __init__(
        ...
        assert (
            0.0 <= split_train_size <= 1.0
        ), "split_train_size does not lie between 0 and 1"
        ...
        if data_set in ["australian", "breast_cancer"]:
            if data_set == "australian":
                ...
                x_train_file = os.path.join(
                    self.root, bca, "breast_cancer_scale_X_tr.csv"
                )
                x_test_file = os.path.join(
                    self.root, bca, "breast_cancer_scale_X_te.csv"
                )
                y_train_file = os.path.join(
                    self.root, bca, "breast_cancer_scale_y_tr.csv"
                )
                y_test_file = os.path.join(
                    self.root, bca, "breast_cancer_scale_y_te.csv"
                )
        ...
        x_full, y_full = np.concatenate((x_train, x_test)), np.concatenate(
            (y_train, y_test)
        )
        ...
        if self.train:
            self.data, self.targets = torch.from_numpy(x_train), torch.from_numpy(
                y_train
            )
        else:
            if self.valid:
                self.data, self.targets = torch.from_numpy(x_valid), torch.from_numpy(
                    y_valid
                )
            else:
                self.data, self.targets = torch.from_numpy(x_test), torch.from_numpy(
                    y_test
                )
