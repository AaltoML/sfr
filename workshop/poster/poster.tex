...
\alert{\bf Full GP.} The predictive equations for a GP model are 
...
\end{align}
But it  requires access to all the data $\rightarrow \color{red}{\mathcal{O}(N^3)}$
...
\end{align*}
using a subset of inducing points $\{\vz_j\}_{j=1}^M$ $\rightarrow \color{cgreen}{\mathcal{O}(M^3) \text{ w/ } (M\ll N)}$

%\heading{Dual parameterization}
\alert{\bf Dual parameterization} for the full posterior GP from Eq.~\ref{eq:gp_pred}:
\begin{align*}
...
\end{align*}
We can approximate the posterior $ {\color{red}{p(\vw \mid \vy)}} \approx \color{cgreen}{q(\weights)} $, and get the dual parameters $\hat{\valpha}, \hat{\vbeta} \in \R^{N}$: 
\begin{align*}
  \alpha_i \approx \hat{\alpha}_i &\coloneqq {\color{cgreen}{\myexpect_{q(\vw)}}} [\nabla_{f}\log p(y_i \mid f) |_{f=f_i}]
  \quad \text{and} \\
  \beta_i \approx \hat{\beta}_i &\coloneqq - {\color{cgreen}{\myexpect_{q(\vw)}}} [ \nabla^2_{ff}\log p(y_i \mid f) |_{f=f_i}]
\end{align*}
Finally, \alert{\it Approximate Posterior} + \alert{\it Sparse GP} $\rightarrow$ \alert{\bf SFR dual parameters}: 
\begin{equation*}
