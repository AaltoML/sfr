...
def random_memorable_points(full_iterator, n_train, n_points, n_classes, label_set):
    memorable_points = {}
    n_points_per_class = int(n_points/n_classes)
    exact_n_points = n_points_per_class*n_classes
    dataset = next(full_iterator)
    X, Y = dataset
    idx_list = np.random.permutation(n_train)
    select_points_num = 0

    for idx in range(len(idx_list)):
        x, y = X[idx_list[idx]], Y[idx_list[idx]]
        x, y = process_data(x, y, label_set)
    ...
    for cid in range(n_classes):
        if cid in memorable_points:
            tmp_r_points = torch.stack(memorable_points[cid], dim=0)
            tmp_r_labels = torch.ones(len(memorable_points[cid]), dtype=torch.long,
                                      device=tmp_r_points.device) * cid
            r_points.append(tmp_r_points)
            r_labels.append(tmp_r_labels)
