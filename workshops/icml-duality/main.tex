...
(e.g. variational inference) whilst capturing information from all data points, and 2) can incorporate new data without retraining (in a computationally efficient manner
...
As Gaussian distributions remain tractable under linear transformations, we can covert the linear model from
...
Importantly, our dual parameterization enables us to 1) sparsify the GP without further optimization, and 2)
incorporate new data without retraining.\looseness-1 % (in a computationally efficient manner).
...
We first compare \our to the Laplace approximation when making predictions with 1) the nonlinear NN (BNN) and 2) the generalised linear model (GLM) in \cref{eq-glm}.
...
Note that this leads to different prior precisions being used for training and inference.
