...
\end{align}
If $\ell(f_\weights(\mathbf{x}_{i}), \mathbf{y}_i) = -\log(p(\mathbf{y} \mid f_\weights(\mathbf{x}_{i}))$ and $\mathcal{R}(\weights) = \|\weights\|^{2}_2$ then we can view \cref{eq-empirical-risk} as the MAP solution to a Bayesian objective, where $p(\vw) = \Norm(\vzeros, \delta^{-1} \MI)$.

\textbf{Neural network to GP} Our goal is to find a kernel from which to build a sparse Gaussian process model. If we had the following function parameterization $g_i(\vx) = \phi^\T(\vx)$ we would could simply convert from weight space to function space the prior under the prior distribution meaning $\kappa(\vx, \vx) = \delta^{-1} \phi(\vx) \phi(\vx)^\T$ see \citep{rasmussen2006gaussian} for details. We use the jacobian of the function $f_\transitionParams(\mathbf{x}_{i})$ and set that to be $\phi(\vx)$. 
...
\begin{align} \label{eq-laplace-approx-function-space}
  ...
  k(\mathbf{x}, \mathbf{x}')
  = \frac{1}{\gamma^{2}} \Jac{\weights^*}{\mathbf{x}}\Jac{\weights^*}{\mathbf{x}'}^\top,
\end{align}
...
with Jacobian given by $\Jac{\weights}{\mathbf{x}} := \nabla_\weights f(\mathbf{x})^\top \in \R^{C \times P}$
%$\mathbf{J}(\cdot) = \odv{\transitionFn(\cdot; \weights)}{\weights}_{\weights=\weights^{*}}$.
