...
\label{app:image}
Similarly to the supervised UCI benchamrk tasks, a single image experiment consist of the following steps: first, training a neural network (a MLP for MNIST, a CNN for FMNIST) in the classification task, using a prior precision $\delta$ in the regularizer term.  This experiment set-up is then replicated for a range of prior precision values and various learning rate, as we perform a grid search over these hyperparameters by optimizing for the validation set NLPD. The results reported in \cref{tbl:imagesuper} used a learning rate of $10^{-4}$ and a prior precision of [INSERT VALUE]. 

For the neural network training, we used the same set-up as in \citet{immer2021improving}, where the batch size is $512$ and the neural networks are trained for $500$ epochs. In contrast to the GP subset implementation in \citet{immer2021improving}, we do not perform scaling of $\delta$ after the neural network has been trained, but use the same prior precision in \our as was used in the regularizer term while training. Avoiding this post-hoc tuning stage allows us to forgo the additional hyperparameter optimization step for the prior precision multiplier. 
