...
class RingBuffer:
    ...
    def init_tensors(self, examples: torch.Tensor, labels: torch.Tensor,
        ...
        for attr_str in self.attributes:
            ...
            if attr is not None and not hasattr(self, attr_str):
                typ = torch.int64 if attr_str.endswith('els') else torch.get_default_dtype()
    ...
    def get_all_data(self, transform: transforms = None):
        populated_portion_length = (self.labels != -1).sum().item()
        all_idx = np.arange(populated_portion_length)
        if transform == None:
            transform = lambda x: x
        ret_tuple = (torch.stack([transform(sample.cpu())
                                  for sample in self.examples[all_idx]]).to(self.device),)
        for attr_str in self.attributes[1:]:
            if hasattr(self, attr_str):
                attr = getattr(self, attr_str)
                ret_tuple += (attr[all_idx], )
        return ret_tuple
    ...
    def get_all_data_old(self, transform: transforms = None) -> Tuple:
        """
