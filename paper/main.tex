...
\begin{table}[t!] 
  \centering\scriptsize
  \caption{Comparisons and ablations on UCI data with negative log predictive density (NLPD\textcolor{gray}{\footnotesize$\pm$std}, lower better). Our sparse \our ($M=256$) is on par with or better than full models (left) and outperforms the GP subset approach of \cite{immer2021improving} (right). Results for methods marked with * are from their original implementation in \cite{immer2021improving}. See \cref{app:uci} for additional tables with comparisons.} 
	\label{tbl:uci}
...
%  \hfill
  ...
  \caption{Comparison of convergence in terms of number of inducing data points $M$ in NLPD (mean over 10 seeds) on UCI classification tasks: \our (thick) vs.\ subsets (thin). The orange lines (\myline{darkorange25512714}) use the GP mean, whereas the blue lines (\myline{steelblue31119180}) the NN MAP estimate as the mean. Our \our's converges fast for all cases.}
  \label{fig:uci}
  %\vspace*{-6pt}
\end{figure}
...
\label{sec:uci}
%
We consider $8$ of the UCI~\cite{UCI} benchmarks, consisting of a range of binary and multi-class classification with varying dataset sizes. We first train two-layer MLPs for each of the classification tasks and follow the setup in \cref{sec:methods} for \our. That is, after training we construct the \our dual and use the resulting model for uncertainty quantification. For comparison, we used the same hyperparameters for neural network training as in the UCI experiments of \cite{immer2021improving}, and similarly performed hyperparameter search over the prior precision $\delta$, and ran the experiment over $10$ random splits.
