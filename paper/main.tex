...
\textbf{Neural Network} A neural network takes ${\mathcal{D}_n = \{(\vx_i, y_i)\}_{i=1}^n}$ and fits a function $f(\mathbf{x}_{i};\transitionParams)$ through the following optimization:
...
\end{align}
If $l(f(\mathbf{x}_{i} ; \transitionParams), y_i) = -\log(p(y \mid f(\mathbf{x}_{i}))$ 
