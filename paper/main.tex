...
>>>>>>> c0a3645 (rewrite of grads)
\begin{equation}
 \ell(h(f), y) = -\log p(y \mid f) =  A(f)  - \langle t(y), f \rangle,
\end{equation}
where $A(f)$ is the log partition function, $h(\cdot)$ is the inverse link function, $t(y)$ are the sufficient statistics which for all our likelihoods is simply $t(y)=y$, finally $f$ denotes the neural network output before the link function. 
