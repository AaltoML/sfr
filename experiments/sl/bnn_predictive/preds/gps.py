...
from experiments.sl.bnn_predictive.preds.likelihoods import BernoulliLh, CategoricalLh
from gpytorch.distributions import MultivariateNormal
from gpytorch.lazy import DiagLazyTensor, lazify, MatmulLazyTensor
        ...
        if self.method == "batched":
            ...
            for i in range(len(X)):
                ...
                grad = torch.autograd.grad(
                    f, (p for p in self.model.parameters() if p.requires_grad)
                )
...
class Parallel_calc_kernel(nn.Module):
    def __init__(
        self, opt_slices_gen, non_opt_slices_gen, n_kernel_groups, with_prior_prec=True
    ):
    ...
    def forward(self, J_row, J_col, calc_diag=False):
        ...
        if not calc_diag:
            kernel_slice = torch.zeros(
                self.n_kernel_groups, J_row.shape[0], J_col.shape[0], device=device
            )
        else:
            kernel_slice = torch.zeros(
                self.n_kernel_groups, J_row.shape[0], device=device
            )
...
class Parallel_calc_a(nn.Module):
    ...
    def forward(self, E, b, c):
        ...
        a = (
            b
            - c
            + (E.view(C * N, N) @ torch.cholesky_solve(self.c_sum, self.M)).view(C, N)
        )
        return (a,)
...
class Parallel_calc_mu(nn.Module):
    def forward(self, K_train_train, K_train_test, pi, gp_f_train_map):
        ...
        alpha = torch.cholesky_solve(
            gp_f_train_map.squeeze(0).unsqueeze(-1), L.squeeze(0)
        ).squeeze(-1)
        if alpha.ndim == 1:
            alpha.unsqueeze_(0)
...
def apply_parallel(
    ...
    if isinstance(module, list):  # replicas already initialized
        ...
        model = module(*module_args).to(
            "cuda:0" if torch.cuda.is_available() and device_ids[0] != -1 else "cpu"
        )
    ...
    for batch_slice in DataLoader(range(total_batch_dim), batch_size=batch_size):
        ...
        if no_grad:
            with torch.no_grad():
                scattered_outputs = nn.parallel.parallel_apply(
                    replicas, scattered_inputs
                )
        else:
    ...
    for i in range(len(outputs)):
        outputs[i] = (
            torch.cat(outputs[i], dim=0) if len(outputs[i]) > 0 else outputs[i][0]
        )
...
def GP_LBFGS(
    ...
    if batch_size_C == -1 or batch_size_C >= C:  # keep L on GPUs
        ...
        cholesky_solve_replicas = nn.parallel.replicate(
            Parallel_cholesky_solve(), device_ids
        )
        scattered_K = nn.parallel.scatter(K_train_train, device_ids)
        L = [
            o[0]
            for o in nn.parallel.parallel_apply(calc_cholesky_replicas, scattered_K)
        ]
    ...
    def lbfgs_closure():
        with torch.no_grad():
            ...
            if batch_size_C == C:
                ...
                scattered_outputs = [
                    o[0]
                    for o in nn.parallel.parallel_apply(
                        cholesky_solve_replicas, list(zip(scattered_fm, L))
                    )
                ]
                a = nn.parallel.gather(scattered_outputs, device, dim=0).squeeze(-1)
            else:
                a = apply_parallel(
                    Parallel_cholesky_solve,
                    (fm.unsqueeze(-1), L),
                    device_ids=device_ids,
                    batch_size=batch_size_C,
                    out_device_id=device,
                ).squeeze(-1)
            loss = -calc_Psi(a, fm)
            if loss > loss_prev:
    ...
    def calc_Psi(a, f):
        return (
            ...
            + likelihood.log_likelihood(
                y_train.to(out_device), (f + gp_train_mean.to(out_device)).t()
            ).sum()
        )
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            a = apply_parallel(
                Parallel_calc_a,
                (E, b, c),
                (M, c.sum(dim=0).view(-1, 1)),
                device_ids=device_ids,
                batch_size=batch_size_C,
                out_device_id=out_device_id,
            )

            f_new = apply_parallel(
                Parallel_einsum,
                (K_train_train, a),
                ("cij,cj->ci",),
                device_ids=device_ids,
...
def GP_predictive(
    ...
    with torch.no_grad():
        ...
        mu = (
            gp_test_mean.to(out_device).t()
            + apply_parallel(
                Parallel_calc_mu,
                (K_train_train, K_train_test, pi, gp_train_f),
                device_ids=device_ids,
                batch_size=batch_size_C,
                out_device_id=out_device_id,
            ).t()
        ...
        if isinstance(likelihood, CategoricalLh):
            ...
            b, c = apply_parallel(
                Parallel_calc_b_c_pred, (E, K_train_test), (M,), batch_size=batch_size_C
            )
            Sigma = apply_parallel(
                Parallel_einsum,
                (b.permute(2, 0, 1), c.permute(2, 1, 0)),
                ("mcn,mnd->mcd",),
                device_ids=device_ids,
                batch_size=batch_size_M,
                out_device_id=out_device_id,
            )
            diag_term = apply_parallel(
                Parallel_einsum,
                (K_train_test.permute(2, 0, 1), b.permute(2, 1, 0)),
                ("mcn,mnc->mc",),
                device_ids=device_ids,
                batch_size=batch_size_M,
                out_device_id=out_device_id,
            )
            Sigma.diagonal(dim1=1, dim2=2).add_(
                K_test_test.to(out_device).t() - diag_term
            )
            ...
            mu = mu.flatten()
            if diag_only:
                Sigma = DiagLazyTensor(
                    K_test_test.flatten().to(device) - contract("nm,nm->m", v, v)
                )
            else:
...
def irls(alpha, m, Ka_fun, alpha_fun, likfun, max_iter=20, tol=1e-3):
    ...
    search_line = lambda alpha, dalpha: brentmin(
        smin_line, smax_line, nmax_line, thr_line, Psi_line, alpha, dalpha
    )
...
def GP_Newton_line_search(
    ...
    def calc_Ka(a):
        if isinstance(likelihood, CategoricalLh):
            ...
            return (
                (K_train_train.squeeze(0).to(device) @ a.to(device).t())
                .t()
                .to(out_device)
            )
    ...
    def calc_a(pi, f):
        if isinstance(likelihood, CategoricalLh):
            ...
            K.diagonal().add_(1.0)
            L = torch.cholesky(K, out=K)
            a = (b - Wsqrt * torch.cholesky_solve(WKb.unsqueeze(-1), L).t()).to(
                out_device
            )
...
def gp_quantities(
    ...
    only_pred_var=True,
    max_ram_gb=10,
    jacobian_method="batched",
    save_jacobians=False,
    train_jacobians=None,
    collapse_groups=False,
    with_prior_prec=True,
    batch_size_per_gpu=-1,
    device_ids=None,
    print_progress=False,
):
    assert (
        isinstance(X_train, (torch.Tensor, torch.utils.data.Dataset)) or train_jacobians
    )
    if X_test is not None:
        assert isinstance(X_test, (torch.Tensor, torch.utils.data.Dataset))
    assert np.any(
        [hasattr(p, "log_prior_var") for p in model.parameters() if p.requires_grad]
    )
    ...
    log_prob = 0
    log_temp = model.log_temp if hasattr(model, "log_temp") else torch.tensor(0.0)

    nn_params = parameters_to_vector(
    ...
    all_vars = set(
        ...
        if p.requires_grad and hasattr(p, "log_prior_var")
    )
    n_groups = len(all_vars)
    optimize_params = (
        set(optimize_params)
        if not isinstance(optimize_params, torch.Tensor)
        else set([optimize_params])
    )
    ...
    total_kernel_size = (
        (n_train**2 if train_jacobians is not None else 0)
        + n_train * n_test
        + (n_test if only_pred_var else n_test**2)
    )
    ...
    for p in model.parameters():
        ...
        if p.log_prior_var in vars_to_optimize:
            if (
                opt_slices[p.log_prior_var]
                and opt_slices[p.log_prior_var][-1].stop == i
            ):
                opt_slices[p.log_prior_var][-1] = slice(
                    opt_slices[p.log_prior_var][-1].start, i + p.nelement()
                )
            else:
                opt_slices[p.log_prior_var].append(slice(i, i + p.nelement()))
        else:
            if (
                non_opt_slices[p.log_prior_var]
                and non_opt_slices[p.log_prior_var][-1].stop == i
            ):
                non_opt_slices[p.log_prior_var][-1] = slice(
                    non_opt_slices[p.log_prior_var][-1].start, i + p.nelement()
                )
            else:
    ...
    jac_size = i
    opt_slices = [
        (kernel_map[var], var.exp().item(), slices)
        for var, slices in opt_slices.items()
    ]
    non_opt_slices = [
        (-1, var.exp().item(), slices) for var, slices in non_opt_slices.items()
    ]
    ...
    K_train = (
        torch.zeros(n_outputs, n_kernel_groups, n_train, n_train)
        if train_jacobians is None
        else None
    )
    ...
    if X_test is not None:
        ...
        K_test_shape = (
            (n_outputs, n_kernel_groups, n_test)
            if only_pred_var
            else (n_outputs, n_kernel_groups, n_test, n_test)
        )
    ...
    for i in device_ids:
        ...
        gpu_models.append(
            Parallel_Jacobian(model, jac_size, 0, jacobian_method).to(device)
        )

    calc_kernel_replicas = nn.parallel.replicate(
        Parallel_calc_kernel(
            opt_slices, non_opt_slices, n_kernel_groups, with_prior_prec
        ),
        device_ids,
    )
    ...
    for idx, output in enumerate(outputs):
        ...
        row_idcs_loader = itertools.chain(
            DataLoader(
                range(n_train),
                batch_size=batch_size if train_jacobians is None else n_train,
                shuffle=False,
            ),
            DataLoader(range(n_test), batch_size=batch_size, shuffle=False)
            if X_test is not None and not only_pred_var
            else (),
        )
        ...
        for row_idcs in row_idcs_loader:
            ...
            if train_jacobians is None or not is_train_row:
                for idcs in DataLoader(
                    range(len(row_idcs)),
                    batch_size=batch_size_per_gpu * len(device_ids),
                ):
                    row_start, row_stop = idcs[0].item(), idcs[-1].item() + 1
                    row_slice2 = slice(
                        row_slice.start + row_start, row_slice.start + row_stop
                    )
                    #                 jac_batch = J_row[row_start: row_stop]
                    scattered_input = nn.parallel.scatter(X[row_slice2], device_ids)
                    J_row_batch, f_row_batch = zip(
                        *nn.parallel.parallel_apply(gpu_models, scattered_input)
                    )
                    J_row[row_start:row_stop] = nn.parallel.gather(
                        J_row_batch, -1, dim=0
                    )

                    if is_train_row and row_slice.start == 0:
                        f_star_train[:, row_slice2] = nn.parallel.gather(
                            f_row_batch, -1, dim=0
                        ).t()[outputs]
            else:
            ...
            total_test_next = total_rows_test + (
                len(row_idcs) if not is_train_row else 0
            )
            col_idcs_loader = itertools.chain(
                # Iterate through train Jacobians that are stored in J_row and compute kernel
                DataLoader(
                    range(total_rows_train, total_train_next),
                    batch_size=batch_size_per_gpu * len(device_ids),
                    shuffle=False,
                )
                if n_train - total_rows_train > 0 and train_jacobians is None
                else (),
                # Calculate train Jacobians that are not in J_row and compute kernel
                DataLoader(
                    range(total_train_next, n_train),
                    batch_size=batch_size_per_gpu * len(device_ids),
                    shuffle=False,
                )
                if n_train - total_train_next > 0 and train_jacobians is None
                else (),
                # Iterate through test Jacobians that are stored in J_row and compute kernel
                DataLoader(
                    range(total_rows_test, total_test_next),
                    batch_size=batch_size_per_gpu * len(device_ids),
                    shuffle=False,
                )
                if X_test is not None and not is_train_row
                else (),
                # Calculate test Jacobians that are not in J_row and compute kernel
                DataLoader(
                    range(total_test_next, n_test),
                    batch_size=batch_size_per_gpu * len(device_ids),
                    shuffle=False,
                )
                if X_test is not None and total_test_next < n_test
                else (),
            )
            ...
            for col_idcs in col_idcs_loader:
                ...
                if is_train_col is is_train_row and col_slice.start < row_slice.stop:
                    row_slice2 = slice(
                        ...
                        col_slice.stop - row_slice.start,
                    )
                ...
                if not is_train_col and only_pred_var and is_first:
                    scattered_outputs = nn.parallel.parallel_apply(
                        calc_kernel_replicas,
                        list(zip(J_col, J_col)),
                        kwargs_tup=({"calc_diag": True},) * len(device_ids),
                    )

                    K_test[idx, :, col_slice].copy_(
                        nn.parallel.gather(
                            scattered_outputs, -1, dim=scattered_outputs[0].ndim - 1
                        )
                    )
                    del scattered_outputs

                if is_first:
                    calc_gp_f_replicas = nn.parallel.replicate(
                        Parallel_calc_gp_function(nn_params), device_ids
                    )
                    scattered_outputs = nn.parallel.parallel_apply(
                        calc_gp_f_replicas, J_col
                    )
                    f_gp_slice = (
                        f_gp_train[idx, col_slice]
                        if is_train_col
                        else f_gp_test[idx, col_slice]
                    )
                    ...
                    if not is_train_col or col_slice.start >= row_slice.stop:
                        ...
                        f_star[:, col_slice] = nn.parallel.gather(f_col, -1, dim=0).t()[
                            outputs
                        ]

                for idcs in DataLoader(
                    range(len(row_idcs)), batch_size=batch_size_per_gpu
                ):
                    jac_left_start, jac_left_stop = idcs[0].item(), idcs[-1].item() + 1
                    row_slice2 = slice(
                        row_slice.start + jac_left_start,
                        row_slice.start + jac_left_stop,
                    )
                    J_row_ = (
                        J_row_saved
                        if train_jacobians is not None and is_train_row
                        else J_row
                    )
                    ...
                    replicated_left_Jacs = [
                        o.J
                        for o in nn.parallel.replicate(
                            Parallel_replicate_jacs(jac_batch.to(default_device)),
                            device_ids,
                        )
                    ]
                    scattered_outputs = nn.parallel.parallel_apply(
                        calc_kernel_replicas, list(zip(replicated_left_Jacs, J_col))
                    )
                    kernel_slice.copy_(
                        nn.parallel.gather(
                            scattered_outputs, -1, dim=scattered_outputs[0].ndim - 1
                    ...
                    if is_train_col is is_train_row:
                        ...
                        K[idx, :, col_slice, row_slice2].copy_(
                            kernel_slice.transpose(-1, -2)
                        )
    ...
    def _collapse_groups(K):
        if len(vars_to_optimize) > 0:
            if K.shape[1] > 1:
                kernel = (
                    K[:, -1].clone()
                    if len(vars_to_optimize) < n_groups
                    else torch.zeros_like(K[:, -1])
                )
    ...
    quantities = {
        ...
        "prior_var": (list(all_vars)[0].exp().item() if len(all_vars) == 1 else None)
        if collapse_groups
        else None,
    }

    if X_test is not None:
        quantities["K_train_test"] = (
            _collapse_groups(K_train_test) if collapse_groups else K_train_test
        )
...
def mc_preds(pred_dist, n_samples=100, batch_size=100):
    assert (
        n_samples % min(n_samples, batch_size) == 0
    ), "Number of samples should be smaller than batch size or divisible by batch size"
    ...
    for i in range(n_batches):
        ...
        new_preds_mean = (total / new_total) * preds_mean + (
            batch_size / new_total
        ) * new_mean
        preds_var_e = (
            (total / new_total) * preds_var_e
            + (batch_size / new_total) * new_var
            + (total * batch_size / new_total**2) * (preds_mean - new_mean) ** 2
        )
...
def init_model_for_gp(
    ...
    stochastic_bias=True,
    device="cuda",
):
    model.likelihood = likelihood
    log_temp = torch.tensor(
        np.log(temperature),
        requires_grad=optimize_temperature,
        device=device,
        dtype=torch.float32,
    )
    ...
    log_var = torch.tensor(
        -np.log(precision),
        requires_grad=optimize_precision,
        device=device,
        dtype=torch.float32,
    )
    ...
    opt_params = [
        p
        for p in [log_var, log_temp, *model.likelihood.parameters()]
        if p.requires_grad
    ]
...
class BernoulliLogitLikelihood(_OneDimensionalLikelihood):
    ...
    def expected_log_prob(self, observations, function_dist, *params, **kwargs):
        ...
        log_prob_lambda = lambda function_samples: torch.nn.functional.logsigmoid(
