...
\label{sec:nn2gp}
%
A common approach to find a distribution over $q(\vw)$ is to perform a Laplace-GGN approximation. The approximation can be seen as taking the Laplace approximation of \cref{eq-empirical-risk} and approximating the   Hessian of $\ell(f_\weights(\mathbf{x}_{i}), \mathbf{y}_i)$ with the Generalized Gauss Newton(GGN) approximation. \citet{immerImprovingPredictionsBayesian2021} proposed to make the following linear approximation
$f^{\textrm{lin.}}_{\vw^*}(\vx; \vw) = f_{\vw^*}(\vx) + \Jac{\weights^*}{\mathbf{x}} \, (\vw - \vw^*)$ of the neural network, with Jacobian $\Jac{\weights}{\mathbf{x}} \coloneqq \nabla_\weights f(\mathbf{x})^\top \in \R^{C \times P}$. The idea

 at the map estimate it was possible to improve predictions from a Laplace-GGN Bayesian neural network.  with Jacobian $\Jac{\weights}{\mathbf{x}} \coloneqq \nabla_\weights f(\mathbf{x})^\top \in \R^{C \times P}$

A Gaussian process characterizes a distribution over functions in terms of the first two moments. The key quantity of a Gaussian process is the prior covariance function (or kernel function) $\kappa(\vx,\vx')$, where $\vx, \vx'$ are input vectors in $\R^{\mathrm{D}}$. We aim to find a kernel based on the neural network to build the Gaussian process posterior.
...
% $\state_{t+1} = \transitionFn(\state_{t}, \action_{t}) + \noise_{t}$ where  $\noise_{t}$
% is i.i.d. transition noise.
The goal of RL is to find the policy \(\pi : \stateDomain \rightarrow \actionDomain\) that maximises the sum of discounted rewards
...
\end{align}
where $\gamma \in [0, 1]$ is a discount factor and $\Pi$ is a set of policies.
...
\quad
  \stateValueFn(\state) &= \mathbb{E} \left[ \sum_{t=0}^{\infty}     \discount^{t} \rewardFn(\state_{t},\action_{t}) \mid \state_{0}=\state \right] \label{eq-fast-update-mpc-old}
\end{align}
\end{subequations}
