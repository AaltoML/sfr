...
\begin{table*}[t!]
  \centering\scriptsize
  \caption{Comparisons and ablations on UCI data with negative log predictive density (NLPD\textcolor{gray}{\footnotesize$\pm$std}, lower better). Our sparse \our (with $M=20\% \text{ of } N$) is on par with the Laplace methods (BNN/GLM) and outperforms the GP subset method when the prior precision $(\delta)$ is tuned (right). Interestingly, when no prior precision ($\delta$) tuning is used (left), \our outperforms all other methods.} %Results for methods marked with * as reported in the original benchmark~\citep{immer2021improving}.} %See \cref{app:uci} for additional tables with comparisons.}
	\label{tbl:uci}
	% \vspace*{-4pt}
...
\todo{update text to say we don't tune prior precision}
