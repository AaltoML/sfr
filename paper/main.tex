...
\label{app:method}
%
Key to our method is the ability to compute the dual parameters in \cref{eq:dual_param}. For general exponential family likelihoods we can derive simpler expressions by rewriting the loss in the natural form:

\begin{equation}
 \ell(h(f), y) = -\log p(y \mid f) =  A(f)  - \langle t(y), f\rangle
\end{equation}
where $A(f)$ is the log partition function and $h(.)$ is the link function and $t(y)$ are the sufficient, given the loss functions we use $t(y) = y$. Taking the first and second derivative of the loss in this form then become:
\begin{equation} \label{eq:}
\nabla_{f}\log p(y_i \mid f) =  A'(f) - y  \quad \nabla^2_{f f}\log p(y_i \mid f_i) = A''(y) 
\end{equation}
The derivative second derivative of the log partition function is easily calculated and the above formulation can be extended to vector input and outputs \ie for the categorical loss, we list the likelihoods we use and their dual parameter below:
%\begin{equation}
%\begin{aligned}
%	\ell(y, \vf_\vw(\vx)) 
%%	&:= - \log p(y | \sigma(f))) \\
...
\begin{tabular}{@{}cccc@{}}
\toprule
Loss        & $\nabla_{f}$ & $\nabla^2_{f f}$ &   $h()$ \\ \midrule
Bernoulli    & $\sigma(f) - y$   & $\sigma(f)(1-\sigma(f))$  & \\
Mse         & $y - f$      & $1$       & \\
Categorical &  $\begin{bmatrix}
		p_1\\ \vdots\\p_k
