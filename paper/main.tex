...
%
\subsection{Computation of the dual parameters for different losses}\todo{just a placeholder}
\label{app:losses}
...
\end{equation}
\todo{check the signs with the main paper}
...
\begin{equation}
	\ell(h(f), y) = \frac{1}{2} (y -f).% -\log \Norm( h(f) | y, \sigma^2) 	= - \log 
\end{equation}
...
In binary classification, we deal with Bernoulli likelihoods. The inverse link function is the sigmoid function $h(f) = \sigma(f) = \frac{1}{1 + e^{-f}}$, and we can then write the negative log likelihood as
\begin{equation}
\begin{aligned}
	\ell(h(f), y) 
...
\begin{equation}
	\nabla_f \ell(h(f), y_i)|_{f=f_i} = - y_i + \sigma(f_i) \quad \text{ and } \quad  \nabla_{f\!f} \ell(h(f), y_i)|_{f=f_i} = \sigma(f_i) (1 - \sigma(f_i)).
\end{equation}

Moving to the multi class classification, the function outputs is no more a scalar but a vector of logits $\bm{f} \in \R^C$. The inverse link function for the categorical loss is the softmax function $\textrm{Softmax}(\bm{f}) : \R^C \mapsto \R^C$ defined as $h(\bm{f}) = \textrm{Softmax}(\bm{f}) = \frac{\exp(\bm{f})}{ \sum^C_{j=1} \exp(\bm{f}_j)} \in \R^C$. If we write the targets $y_i$ as a one-hot $C$-shaped one-hot encoding vector, we can write the negative log likelihood as:
\begin{equation}
\begin{aligned}
	\ell(h(\bm{f}),\vy) &= - \log\left( \vy^\T \textrm{Softmax}(\bm{f})\right)\\
	&= - y_i \cdot \log \left( \left[ \textrm{Softmax}(\bm{f}) \right]_i \right)
\end{aligned}
\end{equation}
where $\left[\textrm{Softmax}(\bm{f})\right]_i = \frac{\exp(\bm{f}_i)}{\sum_{j=1}^{C} \exp(\bm{f}_j)} \in [0, 1]$ defines the $i$\textsuperscript{th} entry of the logits fed through the $\textrm{Softmax}(\cdot)$ function. Finally, we can write the first and second derivatives as follows

\begin{equation}
\begin{aligned}
	\nabla_f \ell(h(f), y_i)|_{f=f_i} &= - y_i + \left[ \textrm{Softmax}(\bm{f}) \right]_i \\
	\nabla_{f\!f} \ell(h(f), y_i)|_{f=f_i} &= \left[\textrm{Softmax}(\bm{f})\right]_i (1 - \left[\textrm{Softmax}(\bm{f})\right]_i).
\end{aligned}
\end{equation}
...
) as $[F\!P]$, this can roughly sketched as $\mathcal{O}(2  E  N[F\!P])$, where $E$ is the number of training epochs and  $N$ the number of training samples. Thus, there is no additional cost involved in the training phase of the neural network.

Given the trained network, we construct the covariance function  which is defined through the NTK~\citep{jacot2018neural}. We relied on the Jacobian contraction implementation using PyTorch~\citep{paszke2019pytorch}. To compute the kernel Gram matrix for a batch of $B$ samples with this implementation has a computational complexity of $\mathcal{O}( B C [F\!P] + B^2 C^2 P)$, where $C$ is the number of NN outputs and $P$ is the number of parameters of the neural network. In \cref{eq:dual_sparse}, we compute $\MB_\vu$ iterating through all the $N$ training points to compute the $\vkzi$ and then use them to compute the outer product, that has a cost of $\mathcal{O}(M^2)$, with $M$ being the number of inducing points following the notation used in the main paper. The total cost of this operation becomes $\mathcal{O}(N M^2)$, which is also the dominating cost of our approach.
...
% role of clamping
% GPU A100 Nvidia
In practical terms, some care needs to be taken during the implementation, both in terms of memory consumption and numerical stability. For the former, we batched over the number of training samples, $N$, which keeps the memory consumption in check without need for any further approximations. The computation of the terms in \cref{eq:dual_sparse_post} require the inversion of an $M \times M$ matrix, that we implemented in a numerically stable manner by using Cholesky factorization. We also introduce a small jitter and clip of the GGN matrix for numerical stability where needed (see discussions in \cref{app:experiments}). 
 We run our experiments GPU clusters equipped with NVIDIA A100 GPUs.
...
The sparse GP posterior $q(\vu)$ is proportional to the product between the prior over the inducing points $p(\vu)$ and a Gaussian term $\Norm(\tilde{\vy}; \vu, \MB_\vu)$, which arises from the sparse approximation to the likelihood. Replacing $\va_i^\top \vu$ with $\vx_i^{\top}\vw$, we can see the similarity between the sparse GP and a linear model where the kernel is the linear kernel. We therefore use the key quadratic term from  $\Norm(\tilde{\vy}; \vu, \MB_\vu)$ as our regularizer as the missing data term, which after we take the $\log$ becomes $(\tilde{\vy} - \vu)^\top \bar{\MB}^{-1}(\tilde{\vy} - \vu)$.  Given that $\tilde{\vy}$ are in the function space we can instead us simply the previous neural network predictions $f^{\textrm{old}}_\weights(\vz_i)$ and save computations.
