...
\textbf{Dual GP} Sparse Gaussian processes \citep{hensman2013gaussian} are a principled way to scale Gaussian processes to large datasets by changing the complexity from $O(n^3)$ where n is number of data points to $O(m^3)$ where $m \ll n$. The key to the complexity saving is to use a sparse distribution
