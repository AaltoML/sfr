...
from experiments.sl.bnn_predictive.preds.predictives import (
    functional_sampling_predictive,
    nn_sampling_predictive,
)
from experiments.sl.bnn_predictive.preds.kron import Kron
from experiments.sl.bnn_predictive.preds.gps import (
    gp_quantities,
    GP_predictive,
    mc_preds,
)
...
class Laplace:
    ...
    def infer(self, train_loader, cov_type="full", dampen_kron=False):
        ...
        if cov_type == "full":
            ...
            N = (
                sum([len(a[0]) for a in train_loader])
            ...
            for X, y in train_loader:
                ...
                # update precision
                precision.update(
                    [p.kflr for p in model.parameters()],
                    factor=hess_factor,
                    batch_factor=M / N,
                )
            precision.decompose()
    ...
    def predictive_samples_glm(self, X, n_samples=1000):
        ...
        return functional_sampling_predictive(
            X, self.model, self.likelihood, self.mu, Sigma, n_samples
        )
    ...
    def predictive_samples_bnn(self, X, n_samples=1000):
        ...
        if type(self.Sigma_chol) is not Kron:
            ...
            Sigma_chol = self.Sigma_chol
        return nn_sampling_predictive(
            X, self.model, self.likelihood, self.mu, Sigma_chol, n_samples
        )
...
class FunctionaLaplace:
    ...
    def infer(
        ...
        self.gp_quantities = gp_quantities(
            self.model,
            X1,
            X2,
    ...
    def predictive_samples(self, n_samples=1000, batch_size=1024, indep=False):
        ...
        if indep:
            ...
            B.diagonal(dim1=1, dim2=2).add_(1.0)
            ...
            gp_train_mean = (
                self.gp_quantities["f_star_train"] - self.gp_quantities["f_gp_train"]
            )
            gp_test_mean = (
                self.gp_quantities["f_star_test"] - self.gp_quantities["f_gp_test"]
            )
            fdist = GP_predictive(
                self.likelihood,
                gp_train_mean,
