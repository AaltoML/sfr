...
\begin{table*}[t!]
  \centering\scriptsize
  \caption{Comparisons and ablations on UCI data with negative log predictive density (NLPD\textcolor{gray}{\footnotesize$\pm$std}, lower better). Our sparse \our ($M=20\% \text{ of } N$) is on par with full models (left) and outperforms the GP subset approach of \citet{immer2021improving} (right).} %Results for methods marked with * as reported in the original benchmark~\citep{immer2021improving}.} %See \cref{app:uci} for additional tables with comparisons.}
	\label{tbl:uci}
	% \vspace*{-4pt}
