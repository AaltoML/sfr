...
An alternative view of the dual variables is that they parameterize approximate likelihoods as was shown in \citep{adam2021dual} we can rewrite the GP posterior in terms of the following approximate normals:
\begin{equation}
 q(\vu) \propto \N(0, \MKzz) \prod_{i=1}^n e^{-\frac{\beta}{2}(y_i - \va_i^\top \vu)^2 } = \N(0, \MKzz) \, e^{(\tilde{\vy} - \vu)^\top\tilde{\MSigma}^{-1}(\tilde{\vy} - \vu)}  
\end{equation}
