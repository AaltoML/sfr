...
The seminal work by \citet{csato2002sparse} (parameterization Lemma~1) gives a parameterization for the posterior process $f_{\textrm{post}}$ that can be found through using Bayes formula the GP prior and the likelihood function. This gives a `dual' parameterization, $\valpha$ and $\vbeta$,
%
...
\end{equation}
%
where the $ij$\textsuperscript{th} entry of the matrix $\MKxx \in \R^{N \times N}$ is $\kappa(\vx_i,\vx_j)$, $\vk_{\vx i}$ denotes a vector where each $j$\textsuperscript{th} element is $\kappa(\vx_i, \vx_j)$, and $k_{ii} = \kappa(\vx_i, \vx_i)$.  \cref{eq:gp_pred} gives the first two moments of the posterior processes. The actual posterior may or may not be a Gaussian process but we can still approximate it with its first two moments.
The dual parameters are the vectors $\valpha, \vbeta \in \R^{N}$, defined as:\looseness-1
%
\begin{equation}
  \label{eq:dual_param}
...
\paragraph{Sparsifying the Neural Network GP}
...
\paragraph{Implementation} We implement all methods in PyTorch~\cite{paszke2019pytorch} and leverage the XXXX libraries.. \todo{add brief details}. Experiments were run on clusters  
...
\begin{table}[t!] 
  \centering\scriptsize
  \caption{Comparisons and ablations on UCI data with negative log predictive density (NLPD\textcolor{gray}{\footnotesize$\pm$std}, lower better). Our sparse \our ($M=256$) is on par with or better than full models (left) and outperforms the GP subset approach of \cite{todo}. See \cref{app:uci} for additional tables with comparisons.} 
	\label{tbl:uci}
...
\label{sec:uci}
%
We consider $8$ of the UCI~\cite{UCI} classification benchmarks as in \cite{immer2021improving}, consisting of a range of binary and multi-class classification with varying dataset sizes. We first train two-layer MLPs for each of the classification tasks and follow the setup in \cref{sec:methods} for \our. After training, we construct the \our dual and use the resulting model for uncertainty quantification. For comparison, we used the same hyperparameters for neural network training as in the UCI experiments of \cite{immer2021improving}, and similarly performed hyperparameter search over the prior precision $\delta$, and ran the experiment over 10 random splits. %Depending on task dimension and complexity, our model can match the neural network MAP in negative log-predictive density (NLPD) even with a low number of inducing points (see \cref{tbl:uci} left).
In \cref{tbl:uci} (left), we compare our model to the NN MAP estimate, mean-field VI, a Bayesian NN, and a GLM model (see \cite{add} for the baselines) in negative log-predictive density (NLPD). We match the predictive performance despite being sparse (here, $M=256$). Results for $M=16,32,64,128,256$ included in \cref{app:uci} with full experiment details. 

In \cref{tbl:uci} (right), we also include an ablation and comparison study with a fixed low number of inducing points ($M=32$). The comparison shows that our sparse method is able to summarize the information of the full data set more efficiently than sparse subset methods, and can match the performance of the full GP even with a low number of inducing points.
...
\begin{wraptable}{R}{.65\textwidth}
  \centering\scriptsize
  \caption{Continual learning experiments. We report mean accuracy${\pm}$std over five runs, and bold based on a $t$-test.\\ $^*$Methods relying on weight regularization.}
...
From \cref{tbl:cl_table_1}, it is clear that the objective methods completely fail compared to the function-space methods on S-MNIST and S-FMNIST, where the tasks are composed of disjoint set of classes, but they are still able to retain previous performance on P-MNIST where the challenge is to contrast the domain shift over the tasks. %, commonly referred as Domain Incremental Learning. (opposed to the Class-Incremental Learning for S-MNIST and S-FMNIST)
Considering the function-space methods, we can see that \our achieves the best results on both settings on S-MNIST, where the condensing the information coming from the entire task datasets $\dataset_t$ is particularly effective, when using fewer points per task.
On S-FMNIST, our method obtains close results to DER, which regularizes the model by taking the mean squared error between current and old function outputs without accounting the covariance structure of the loss but assumes an i.i.d. prior similar to our L2 ablation. Moreover, their method exploits reservoir sampling \citep{vitter1985random} to obtain a continuously updated set inducing points.
S-FSVI obtains the best on P-MNIST, but with the downside of requiring high computational costs, compared to \our. \todo{on PMNIST our method is not really super-fast, so that could be not the best thing to say here} % we could also say that P-MNIST is known to be a bad/ill-posed dataset for CL 
