...
class _OurSingleTaskVariationalGP(ApproximateGP):
    def __init__(
        ...
        self.train_inputs = (
            [train_inputs] if train_inputs is not None else [init_points]
        )
        self.train_targets = (
            train_targets if train_targets is not None else init_targets
        )
    ...
    def get_fantasy_model(
        ...
        **kwargs,
        ...
        full_inputs = torch.cat(
            [torch.tile(inducing_points, X.shape[:-2] + (1, 1)), X], dim=-2
        )
        ...
        for _ in range(self.num_online_updates):
            ...
            grad_mu_2 = K_uf.matmul(
                DiagLazyTensor(grad_nat_2).matmul(K_uf.swapdims(-1, -2))
            )
            ...
            fantasy_var_dist = (
                fantasy_model.variational_strategy._variational_distribution
            )
            with torch.no_grad():
...
class OurSingleTaskVariationalGP(_OurSingleTaskVariationalGP, GPyTorchModel):
    ...
    def posterior(
        ...
        **kwargs,
        ...
        if observation_noise and not isinstance(
            self.likelihood, _GaussianLikelihoodBase
        ):
