...
In this section, we demonstrate how sequential learning methods can benefit from our function-space representation.
...
\begin{align} \label{eq-fast-update-mpc}
  \policy^{\text{Greedy}}(\state) &= \arg \max_{\pi \in \Pi} J(f_{\mathbf{w}^{*}}, \pi) \quad   \\
  \policy^{\text{PS}}(\state) &= \arg \max_{\pi \in \Pi} J(\hat{f}, \pi)
\quad \text{s.t. } \hat{\transitionFn} \sim p(\transitionFn_{\mathbf{w}} \mid \dataset),
\end{align}
