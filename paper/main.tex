...
Quantifying uncertainty in a Bayesian framework means finding a posterior of $p(\vw \mid \dataset)$. The posterior represents the uncertainty over our parameters based on the prior and the data. The posterior structure directly relates to the optimization loss around the MAP weights $\vw^*$. Furthermore, the landscape in the weight space induces functions produced by the neural network's deterministic mapping. If one was to sample from the weight posterior, the corresponding functions created can be viewed as perturbed versions of the $f_{\vw^*}$. In most applications, we care about predictions from the neural network, so the distribution in the function space not in the weight space. 

Although the true posterior is intractable given the non-linearities of the neural network sampling techniques are available such as Hamiltonian Monte-Carlo (HMC), The uncertainty plotted \cref{fig:banana} shows the induced function space posterior by sampling through the model. However, in most cases, sampling techniques like HMC are not available, given the computational burden.

A common approach is to approximate the correlation structure of a distribution centred at the MAP estimate as done in the Laplace-GGN \citep{khan2019approximate, daxberger2021laplace, maddox2021fast}. The Laplace GGN takes the MAP solution and approximates the Hessian of the loss function  $\ell(f_\weights(\mathbf{x}_{i}), \mathbf{y}_i)$ with the GGN approximation. Another way of viewing the approximation is that we build the following approximate model $\hat{f}_\weights(\mathbf{x}_{i}) = \Jac{\weights_*}{\mathbf{x}_i} \vw_i $  where $\Jac{\weights}{\mathbf{x}} \coloneqq \nabla_\weights f(\mathbf{x})^\top \in \R^{C \times P}$ is the Jacobian as was pointed out in \citep{khan2019approximate}. Using the Hessian of the approximate model, we arrive at the Laplace-GGN posterior for $q(\vw)$. As pointed out in \citet{khan2019approximate}, a Gaussian posterior covariance approximation in the weight space implies a covariance structure in the function space.

In this paper we are concerned with learning posterior directly in the function space that is having a $q(\vf)$ instead of a $q(\vw)$. We choose is to use a Gaussian process an extension of the multivariate normal that characterizes a distribution over functions in terms of the first two moments, a mean $\mu(\cdot)$ and covariance function $\kappa(\cdot,\cdot)$ (kernel). We aim to find a kernel and mean function based on the neural network to build the Gaussian process posterior.  A linear function in terms of parameters can be converted from the weight space to the function space (see Ch.~XXX in \cite{rasmussen2006gaussian} as follows:
...
We can therefore use the approximate linear model defined by the GGN approximation to define our GP derived from the trained neural network,
...
\end{equation}
where the kernel is the so-called Neural Tangent Kernel (NTK, \cite{jacot2018neural}), given we have a kernel function, we can then combine it with the $\dataset$ to form the posterior. In \citep{khan2019approximate} they took a similar approach but instead of fitting the GP posterior to the actual data they adjusted the data to be pseudo data. Similarly, using a first order approximation of the neural network function to form a GLM model \citet{immerImprovingPredictionsBayesian2021} also arrives at the same NTK function for the GP derived from the neural network, however a different mean function. Both previous approaches have attempted to adjust the GP posterior mean function to ensure the predictions are from the neural network $f_{\vw^*}(\vx)$. In our method, we wish to make predictions from the derived GP and thus do not make such adjustments. We now present the general formulation of the functional representation of our trained neural network
...
\begin{equation}
	\mathcal{R_\textit{SFR}}(\mathbf{w}) = \sum_{s=1}^{t-1} \frac{1}{m} \left[\left(f_{\vw}(\MZ_{s}) - f_{\vw_{s}}(\MZ_s) \right)^\T \bar{\MB}^{-1}_{s} \left(f_{\vw}(\MZ_{s}) - f_{\vw_{s}}(\MZ_s) \right) \right]
\end{equation} 
with $f_\vw(\MZ_{s}), f_{\vw_s}(\MZ_s) \in \R^m$ are vectors of the outputs of the neural networks with the current weights $\vw$ and stored function outputs after task $s$
