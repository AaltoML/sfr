...
\begin{abstract}
Deep neural networks lack uncertainty estimates, struggle to incorporate new data, and fail to retain previous knowledge.
We present a method that overcomes these limitations by converting a weight-space neural network to a low-rank function-space representation, through a dual parameterisation.
In contrast to previous work, we model the joint distribution over the full data set and not just a subset.
Our method provides uncertainty estimates and enables us to incorporate new data without retraining, whilst retaining predictive performance.
We also show that our function-space representation can be used as a low-rank summary of previous data in continual learning problems.
\end{abstract}
