...
# Set up plotting
def plot(sfr, ax=None, vmax=None, network=None, plotSVGP=True, save=None, data=None):
    ...
    if hasattr(sfr, "Z") and plotSVGP == True:
    ...
    if network is not None:
        ...
        if plotSVGP == False:
            ...
            ax.imshow(
                y_net.reshape(*xtest.shape).transpose(),
                extent=limits,
                origin="lower",
                cmap=cmap_nn,
                vmin=-1,
...
plot(
    ...
    plotSVGP=False,
    save="figs/banana-nn",
    data=(X_train.numpy(), Y_train.numpy()),
)
