...
%\crefname{table}{Tab.}{Tabs.} % Let's leave table as is
...
% Sampling methods that characterize the posterior with a finite set of samples in weight-space are general-purpose, but computationally heavy andimpractical for downstream applications.
...
% where $q(\vf) = \mathcal{N} \left( \myexpect_{p(f_i \mid\vy)}[f_i], \mathrm{Var}_{p(f_i \mid \vy)}[f_i] \right)$ is the predictive GP posterior given by,
...
Assuming a zero mean function, the sparse GP predictive posterior is
\begin{subequations}  \label{eq-svgp-pred}
\begin{align}
  % \myexpect_{q_{\vu}(f_i)}[f_i] &= f_{\vw^{*}}(\vx_{i}) + \vk_{\vz i}^{\top}\MKzz^{-1}(\vm - f_{\vw^{*}}(\MZ)), \\
  \myexpect_{q_{\vu}(f_i)}[f_i] &= \vk_{\vz i}^{\top}\MKzz^{-1}\vm, \\
  \mathrm{Var}_{q_{\vu}(f_i)}[f_i] &= k_{ii} - \vk_{\vz i}^\top ( \MKzz^{-1} -  \MKzz^{-1}\MS  \MKzz^{-1}) \vk_{\vz i},  \nonumber
\end{align}
\end{subequations}
where $\MKzz$ and $\vkzs$ are defined similarly to $\MKxx$ and $\vk_{\vx i}$ but over the inducing points $\MZ$.%, instead of the full data set $\dataset$.
...
Importantly, our dual parameterization enables us to {\em (i)}~sparsify the GP without further optimization, and {\em (ii)}~incorporate new data without retraining.\looseness-1 % (in a computationally efficient manner).
...
\begin{figure*}[t]
  ...
  \setlength{\figureheight}{\figurewidth}
  \pgfplotsset{axis on top,scale only axis,y tick label style={rotate=90}, x tick label style={font=\Large},y tick label style={font=\Large},title style={yshift=-4pt,font=\LARGE}, y label style={font=\Large},x label style={font=\LARGE},grid=major, width=\figurewidth, height=\figureheight}
  \pgfplotsset{grid style={line width=.1pt, draw=gray!10,dashed}}
  \pgfplotsset{xlabel={$M$ as \% of $N$},ylabel style={yshift=-5pt},xlabel style={yshift=-5pt}}
  %
...
% We first compare \our to other posthoc Bayesian deep learning approaches.
...
This is interesting and we believe it stays more true to Bayesian inference.% (as tuning the prior precision is equivalent to altering the prior after calculating the posterior).
...
(and posterior) being a locally linear approximation of the NN.\looseness-1
