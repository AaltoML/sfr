    ...
    def next_task(self):
        if self.cur_iter >= self.max_iter:
            ...
            for class_index in range(np.size(self.sets[self.cur_iter])):
                ...
                # Initialise next_y_train to zeros, then change relevant entries to ones, and then stack
                next_y_train_interm = np.ones((len(train_id)), dtype='int64')
                if class_index == 0:
                    next_y_train = next_y_train_interm * self.sets[self.cur_iter][class_index]
                else:
                    # next_y_train_interm += 1
                    next_y_train =  np.concatenate((next_y_train, 
                                                    np.ones((len(train_id)), dtype='int64') * self.sets[self.cur_iter][class_index]), 
                                                    axis=0)
                ...
                next_y_test_interm = np.ones((len(test_id)), dtype='int64')
                if class_index == 0:
                    next_y_test = next_y_test_interm  * self.sets[self.cur_iter][class_index]
                else:
                    next_y_test = np.concatenate((next_y_test,
                                                  np.ones((len(test_id)), dtype='int64') * self.sets[self.cur_iter][class_index]), 
                                                  axis=0)
...
# class ContinualFMNIST(FashionMNIST):
#     def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
#         super().__init__(root, train, transform, target_transform, download)
#         self.task_id = torch.zeros_like(self.targets)
    
#     def __getitem__(self, index):
#         item = super().__getitem__(index)
#         return (*item, self.task_id[index].item())

class SplitFMnistGenerator():
    def __init__(self):
        data_transform = tr.Compose([
            # tr.ToPILImage(),
            tr.ToTensor(),
            tr.Lambda(lambda x: x.reshape(-1))
        ])
    ...
    def get_dims(self):
        # Get data input and output dimensions
        return self.train_dataset[0].shape[1], self.out_dim

    def next_task(self):
        if self.cur_iter >= self.max_iter:
            raise Exception('Number of tasks exceeded!')
        else:
            # Take the elements in the two classes for the current iter
            all_idx_train = torch.arange(len(self.train_dataset))
            task_idx_train = np.logical_or(
                self.train_dataset.targets.numpy() == self.sets[self.cur_iter][0],
                self.train_dataset.targets.numpy() == self.sets[self.cur_iter][1]
            )
            task_idx_train = all_idx_train[task_idx_train]
            train_task_subset = Subset(self.train_dataset, indices=task_idx_train)
            
            all_idx_test = torch.arange(len(self.test_dataset))
            task_idx_test = np.logical_or(
                self.test_dataset.targets.numpy() == self.sets[self.cur_iter][0],
                self.test_dataset.targets.numpy() == self.sets[self.cur_iter][1]
            )
            task_idx_test = all_idx_test[task_idx_test]
            test_task_subset = Subset(self.test_dataset, indices=task_idx_test)
            
            self.cur_iter += 1
            
            return train_task_subset, test_task_subset, self.sets[self.cur_iter-1]
