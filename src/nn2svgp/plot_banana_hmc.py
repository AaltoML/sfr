...
n = X.shape[0]
train_set = banana_dataset([X[i,0] for i in range(n)], [X[i,1] for i in range(n)], [Y[i] for i in range(n)])
...
def custom_collate(batch):
    ...
    return torch.as_tensor(data).reshape((-1,2)), torch.as_tensor(labels).reshape((-1,1)).float()
...
def plot(hmc_pred,ax=None, vmax=None, network=None, plotSVGP=True, save=None, data=None):
    ...
    ax.contour(xtest, ytest, mu.reshape(*xtest.shape), levels=[0.5],
