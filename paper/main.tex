...
\label{sec:uci}
%
We consider the same 8 UCI~\cite{UCI} classification benchamrks tasks and setup as were considered in \cite{todo}. We use a two-layer MLP for all tasks and follow the setup in \cref{sec:methods} for \our. After training, we construct the \our dual and use the resulting model for uncertainty quantification. We performed hyperparameter search over the prior precision $\delta$, and ran the experiment over 10 random splits. %Depending on task dimension and complexity, our model can match the neural network MAP in negative log-predictive density (NLPD) even with a low number of inducing points (see \cref{tbl:uci} left).
In \cref{tbl:uci} (left), we compare our model to the NN MAP estimate, mean-field VI, a Bayesian NN, and a GLM model (see \cite{add} for the baselines) in negative log-predictive density (NLPD). We match the predictive performance despite being sparse (here, $M=$). Results for $M=16,32,64,128,256$ included in \cref{app:uci} with full experiment details. 

In \cref{tbl:uci} (right), we also include an ablation and comparison (to \cite{todo}) study with a fixed low number of inducing points ($M=$). The comparison shows that our sparse method is able to summarize the information of the full data set more efficiently than sparse subset methods.
