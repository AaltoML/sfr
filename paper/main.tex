...
\textbf{Neural network to GP} Our goal is to find a kernel from which to build a sparse Gaussian process model. If we had the following function paramaterizatoin $f_i = \phi^\T(\vx)$ we would could simply convert from weight space to function space the prior 
