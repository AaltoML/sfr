...
with the GGN.
Following the work of \citet{khan2019approximate}, we can view this as a linear approximation of the NN, as $f_{\weights^*}(\vx) \approx
%g_{\weights}(\vx) =
...
Early work by \citet{csato2002sparse} showed that Expectation Propagation gives rise to a dual paramaterization.
...
\end{align}
%
Substituting \cref{eq:dual_param_laplace} into \cref{eq:gp_pred}, we obtain our GP based on the converged NN. Again, this is similar to what was derived in \citet{immer2021improving} for the posterior variance function.
However, they use the NN's prediction $f_{\vw^*}(\cdot)$ for the posterior mean and do not emphasize the significance of the dual parameters.
The problem with the predictions in \cref{eq:gp_pred} is that they incur a cost of $O(N^3)$, which limits its application on large data sets.
...
\begin{figure*}[t]
  ...
  \caption{Comparison of convergence in terms of number of inducing points $M$ in NLPD (mean$\pm$std over 5 seeds) on UCI classification tasks: \our (\myline{steelblue31119180}) vs. GP subset (\myline{darkorange25512714}). Our \our converges fast for all cases showing clear benefits of its ability to summarize all the data onto a sparse set of inducing points. The number of inducing points $M$ are specified as a percent of the number of training data $N$.}
  \label{fig:uci}
  %\vspace*{-6pt}
\end{figure*}
...
We project the dual parameters onto the inducing points sparse GP using the dual parameters,
...
We report results when tuning (\cref{tbl:uci} right) and not tuning (\cref{tbl:uci} left) the prior precision $\delta$ after performing inference.
When tuning the prior precision $\delta$, \our matches the performance of the Laplace methods (BNN/GLM).
Interestingly, also \our performs well without tuning the prior precision $\delta$, unlike the Laplace methods (BNN/GLM).
...
This demonstrates that \our's sparse representation captures information from the entire data set and as a result can use fewer inducing points than the GP subset.
