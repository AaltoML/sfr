...
\textbf{Neural Network} In supervised learning, given a data set $\dataset = \{(\mathbf{x}_{i} , \mathbf{y}_{i})\}_{i=1}^{n}$, with $\mathbf{x}_i \in \inputDomain$ and $\mathbf{y}_i \in \outputDomain$, the weights $\transitionParams \in \R^{P}$ of a neural network
...
\begin{align} \label{eq-empirical-risk}
  \transitionParams^{*} = 
  \argmin_{\transitionParams} \mathcal{L}(\dataset,\transitionParams) =
  \argmin_{\transitionParams} \sum_{i=1}^{n} \ell(f_\transitionParams(\mathbf{x}_{i}), \mathbf{y}_i) + \delta \mathcal{R}(\transitionParams).
\end{align}
If $\ell(f_\transitionParams(\mathbf{x}_{i}), \mathbf{y}_i) = -\log(p(\mathbf{y} \mid f_\transitionParams(\mathbf{x}_{i}))$ and $\mathcal{R}(\transitionParams) = \|\transitionParams\|^{2}_2$ then we can view \cref{eq-empirical-risk} as the MAP solution to a Bayesian objective, where $p(\vw) = \Norm(\vzeros, \delta^{-1} \MI)$.

\textbf{Neural network to GP} Our goal is to find a kernel from which to build a sparse Gaussian process model. If we had the following function parameterization $g_i(\vx) = \phi^\T(\vx)$ we would could simply convert from weight space to function space the prior under the prior distribution meaning $\kappa(\vx, \vx) = \delta^{-1} \phi(\vx) \phi(\vx)^\T$ see \citet{rasmussen2006gaussian} for details. We linearise the neural network function  

% The Laplace approximation allows us to build a posterior approximation $q(\vw)$ from the optimized objective \cref{eq-empirical-risk}. Central to the approximation is how to form the hessian of \cref{eq-empirical-risk} a common approach is to use the the GGN approximation which would result in the following posterior $q(\vw;\vw^*, \MSigma_{\textrm{GGN}})$ where 

As was pointed out in \citep{immerScalable2021} we can view the approximation in two steps.
...
\begin{align} \label{eq-laplace-approx-function-space}
  ...
  g(\vx_i) \sim \mathcal{N} \left( \mu(\vx_i), k(\vx_i, \vx_i) \right) 
  ...
  k(\mathbf{x}, \mathbf{x}')
  = \frac{1}{\gamma^{2}} \Jac{\transitionParams^*}{\mathbf{x}}\Jac{\transitionParams^*}{\mathbf{x}'}^\top,
\end{align}
...
with Jacobian given by $\Jac{\transitionParams}{\mathbf{x}} := \nabla_\transitionParams f(\mathbf{x})^\top \in \R^{C \times P}$
%$\mathbf{J}(\cdot) = \odv{\transitionFn(\cdot; \transitionParams)}{\transitionParams}_{\transitionParams=\transitionParams^{*}}$.
...
\textbf{Dual Gaussian Process} A Gaussian process (GP) is a prior over functions $f \sim \GP(\mathbf{0}, \kappa(\cdot, \cdot))$. The prior is characterized by a covariance kernel function $\kappa(\vx,\vx')$, where $\vx, \vx'$ are input vectors in $\R^{\mathrm{D}}$. Once combined with a data set ${\mathcal{D} = \{(\vx_i, y_i)\}_{i=1}^n}$ with $n$ examples, each modelled by the likelihood $p(\vy_i \mid \vf_i)$ at function $\vf_i = f(\vx_i)$\, we can form a posterior proccess using multivariate conditioning. The drawbacks of GPs reside in their lack of scaling to large datasets and non-tractability with non-Gaussian likelihoods.
