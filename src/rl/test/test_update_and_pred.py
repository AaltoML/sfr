...
def train(cfg: DictConfig):
    ...
    print("y: {}".format(svgp.predict(X_test)))
    ...
    # src.models.svgp.update(svgp=svgp, likelihood=svgp.likelihood)(data_new=data_new)
    ...
    # src.models.svgp.update(svgp=svgp, likelihood=likelihood)(data_new=data_new)
    svgp.update(data_new=data_new)
    mean_new, var_new, noise_var = svgp.predict(X_test)
    ...
    X_new = torch.linspace(5.0, 7.0, 10, dtype=torch.float64).reshape(-1, 1)
    # X_new = torch.linspace(2, 3.5, 100, dtype=torch.float64)
    Y_new = func(X_new, noise=True)
    data_new = (X_new, Y_new)
    # src.models.svgp.update(svgp=svgp, likelihood=likelihood)(data_new=data_new)
    svgp.update(data_new=data_new)
    mean_new, var_new, noise_var = svgp.predict(X_test)
    print("pred mean_new 2 {}".format(mean_new.shape))
    print("pred var_new 2 {}".format(var_new.shape))
    plot("gp_2.pdf")
