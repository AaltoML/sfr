...
%  \end{subfigure}
  ...
  \pgfplotsset{xlabel={Input, $x$},axis line style={rounded corners=3pt}}
  % Set figure 
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    % Arrows
    \node[myarrow] (p0-arr) at ($(p0) + (0em,1.5em)$) {};
    \node[myarrow] (p1-arr) at ($(p1) + (0em,1.5em)$) {};
    % Arrow labels
...
\begin{figure}[t!]
  ...
  \begin{tikzpicture}[outer sep=0,inner sep=0]
    ...
    \node[font=\scriptsize\sc,color=white] at (second-arr) {new data};   
...
\textbf{Neural Network} Given the data set $\dataset_n = \{\mathbf{x}_{i} \in \inputDomain, y_{i} \in \outputDomain\}_{i=1}^{N}$,
...
\begin{align} \label{eq-empirical-risk}
  \transitionParams^{*} = \arg \min_{\transitionParams \in \R^{P}} \mathcal{L}(\dataset;\transitionParams) = \arg \min_{\transitionParams \in \R^{P}} \left(
  \sum_{i=1}^{N} l(f(\mathbf{x}_{i} ; \transitionParams), y_i) + \frac{1}{\delta}R(\transitionParams)  \right).
\end{align}
If $l(f(\mathbf{x}_{i} ; \transitionParams), y_i) = -\log(p(y \mid f(\mathbf{x}_{i}))$ and $R(\transitionParams) = \|\transitionParams\|^{2}$ then we can view \cref{eq-empirical-risk} as the MAP solution to a Bayesian objective, where $p(\vw) = \N(\vzeros,\delta\MI)$.

\textbf{Laplace approximation} The Laplace approximation allows us to build a posterior approximation $q(\vw)$ from the optimized objective \cref{eq-empirical-risk}.
...
Here, $\MKzz$ to denote the $m \times m$ matrix with $\kappa(\vx_i,\vx_j)$ as the $ij$\textsuperscript{th} entry, Similarly, $\vk_{\vx i}$ denotes a vector where each $j$\textsuperscript{th} element is $\kappa(\vx_i, \vx_j)$, and $\kappa_{ii} = \kappa(\vx_i, \vx_i)$.  The dual parameters consist of an $m$-length vector $\valpha$ and $m\times m$-size matrix $\vbeta$, defined as
\begin{equation} 
