...
\begin{abstract}
Sequential learning paradigms such as Continual Learning (CL) or Reinforcement Learning (RL) pose a challenge for gradient-based deep learning techniques as they struggle to incorporate new data and retain previous knowledge.
Existing methods for converting neural networks from weight to function space allow a probabilistic treatment of the distribution over the function learned by the neural networks but are computationally expensive.
We propose a method that converts a neural network to a low-rank functional representation as a sparse Gaussian process. 
With this approach, we can build a compact representation of the function encoded by the neural network that can replace previous data in continual settings and be used for fast adaptation in RL, avoiding full retraining of the model. 
\end{abstract}
...
\begin{figure}[t!]
  ...
  \begin{tikzpicture}[outer sep=0,inner sep=0]
    ...
    % The update
    \addfig{2.2\figurewidth,0}{banana-update}


     % The arrow
     \tikzstyle{myarrow} = [draw=black!80, single arrow, minimum height=12mm, minimum width=2pt, single arrow head extend=4pt, fill=black!80, anchor=center, rotate=0, inner sep=5pt, rounded corners=1pt]
     \node[myarrow] (first-arr) at ($(banana-nn)!0.5!(banana-nn2svgp)$) {};
     \node[myarrow] (second-arr) at ($(banana-nn2svgp)!0.5!(banana-update)$) {};

     % Arrow labels
     \node[font=\scriptsize,color=white] at (first-arr) {nn2svgp};
     \node[font=\scriptsize,color=white] at (second-arr) {new data};   
     ...
     \node[anchor=north, font=\small] at ($(banana-update) + (0,-.55\figureheight)$) {Update without re-training};      
...
\textbf{Neural Network in weight space}


\textbf{Laplace approximation}


\textbf{Dual Gaussian Process} A Gaussian process (GP) is a prior over functions $f \sim \GP(0, \kappa)$. The prior is characterized by a covariance kernel function $\kappa(\vx,\vx')$, where $\vx, \vx'$ are input vectors typically in $\R^{\mathrm{D}}$. Once combined with a data set ${\mathcal{D}_n = \{(\vx_i, y_i)\}_{i=1}^n}$ with $n$ examples, each modeled by the likelihood $p(y_i \mid f_i)$ at function $f_i = f(\vx_i)$, we can form a posterior proccess using multivariate conditioning. The drawbacks to GPs are their lack of scaling to large datasets and non-tractability with non-Gaussian likelihoods.

Sparse Gaussian processes \citep{hensman2013gaussian} are a principled way to scale Gaussian processes to large datasets by changing the complexity from $O(n^3)$ to $O(m^3)$ with $m$ the number of inducing points, and typically $m \ll n$. The key insight is to use a sparse approximating distribution $q(\vu; \vm, \MV)$, where $\vm \in \R^{m}$ and $\MV \in \R^{m \times m}$, and find its parameters through maximizing the evidence lower bound (ELBO). The distribution at all over points is then defined as $q_{\vu}(f) = \int p(f \mid \vu) q(\vu) d\vu$. The dual parameterization \cite{adam2021dual} states that we can decompose the optimal sparse Gaussian process posterior as follows:
\begin{equation}
   \myexpect_{q_{\vu}(f)}[f_i] = \vkzs^{\T} \valpha , \quad 
   \textrm{Var}_{q_{\vu}(f)}[f_i]  = \kappa_{ii} - \vkzs^\top [\MKzz^{-1} - (\MKzz + \vbeta)^{-1} ]\vkzs.
\end{equation}

Here, we omit the dependency of the kernel on $n$, and simply use $\MKxx$ to denote the $n \times n$ matrix with $\kappa(\vx_i,\vx_j)$ as the $ij$\textsuperscript{th} entry, computed for all $\vx_i \in\vX$ where $\vX = (\vx_1,\ldots,\vx_n)$. Similarly, $\vk_{\vx i}$ denotes a vector where each $j$\textsuperscript{th} element is $\kappa(\vx_i, \vx_j)$, and $\kappa_{ii} = \kappa(\vx_i, \vx_i)$. 


The dual parameters consist of an $m$-length vector $\valpha$ and $m\times m$-size matrix $\vbeta$, defined as
\begin{equation} 
...
where $\tilde{\lambda}_i = \alpha_i - \myexpect_{q_{\vu}(f)}[f_i] \hat{\beta}_i$ with $(\hat{\alpha}_i, \hat{\beta}_i)$ defined as,
\begin{equation}
 \alpha_i = \myexpect_{q_{\vu}(f_i)} [\nabla_{f_i}\log p(y_i \mid f_i)], \quad
   \beta_i = \myexpect_{q_{\vu}(f_i)} [ -\nabla^{2}_{f_i}\log p(y_i \mid f_i)],
\label{eq:svgp_dual}
\end{equation}

The key quantities we need to build our sparse GP are $(\alpha_i, \beta_i)$ and a kernel function $\kappa$  
...
\begin{table}[t!] 
  \centering\scriptsize
  \caption{TODO: CL Experiments. $^*$ Methods relying only on weight regularization. \color{blue}{Results from \cite{rudner2022continual}.} \color{red}{Preliminary results need more runs or hyperparam tuning}}
