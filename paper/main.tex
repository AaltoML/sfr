...
Although the true posterior is intractable given the non-linearities of the neural network, one can restore to sampling techniques such as Hamiltonian Monte-Carlo (HMC). However, even in this case, the high computational costs make them impractical. In \cref{fig:banana}, we show a qualitative example of the induced function space posterior obtained with our method compared to the posterior obtained through HMC sampling on the Banana toy dataset. \todo{make it more straightforward that we are computationally cheaper?}
...
Following the work of \citep{khan2019approximate}, we can view this approximation as building an approximate linear model of the neural network as $\hat{f}_\weights(\mathbf{x}) = \Jac{\weights_*}{\mathbf{x}} \, \weights$, where $\Jac{\weights}{\mathbf{x}} \coloneqq \nabla_\weights f(\mathbf{x})^\top \in \R^{C \times P}$ is the Jacobian at the MAP. Using the Hessian of the approximate model, we arrive at the Laplace-GGN posterior for $q(\vw)$. As pointed out in \citet{khan2019approximate}, a Gaussian posterior covariance approximation in the weight space implies a covariance structure in the function space. \todo{the last two sentences are not linked to the previous ones}

In this paper, we are concerned with learning the posterior $q(\vf)$ directly in the function space, instead of $q(\vw)$. We choose to use a Gaussian process, an extension of the multivariate normal that characterizes a distribution over functions in terms of the first two moments, a mean $\mu(\cdot)$ and covariance  (or kernel) function $\kappa(\cdot,\cdot)$. We aim to these two functions based on the neural network to then build the Gaussian process posterior. A function linear with respect to its parameters can be converted from the weight space to the function space (see Ch.~XXX in \cite{rasmussen2006gaussian} as follows: \todo{this part needs to be improved the linear function pops out of nowhere}
\begin{equation} \label{eq:weight_func}
g_\weights(\mathbf{x}_{i}) = \phi^\top\!(\vx) \, \vw \implies \mu(\vx) = 0 \quad \kappa(\mathbf{x}, \mathbf{x}') = \frac{1}{\delta} \phi(\vx)^\top \phi(\vx)
\end{equation}

Therefore, we can use the approximate linear model defined by the Laplace-GGN approximation to define our GP derived from the trained neural network,
\begin{equation} 
