...
\our is a more principled approach since it sparsifies the resultant GP into a low-rank approximation to ensure information coming from the entire task dataset is not lost. %, essentially what the sparse dual variables do.   \todo{this term is replacing the missing data}
...
\textbf{Predictions:}~Does our method's ability to consider the full data set offer benefits over subset function-space methods? (\cref{sec:uci,sec:image}) How do predictions with our sparse function-space approximation compare to weight-space approximations? 
%\textbf{Function-space updates:} How fast are our function-space updates relative to retraining from scratch? Do they improve predictive performance? Are they as good as retraining from scratch? \todo{what do we have on this?}
...
\begin{table}[t!] 
  \centering\scriptsize
  \caption{Negative log predictive density (NLPD) (lower better) for SVGP (sparse GP mean and variance), SVGP NN (NN mean and sparse GP variance), GP subset (subset GP mean and variance) and GP NN subset (NN mean and GP subset variance). The \our on the left side of the table used 256 inducing points } 
	\label{tbl:uci}
	% Control table spacing
	\renewcommand{\arraystretch}{1.}
	\setlength{\tabcolsep}{1pt}
	\setlength{\tblw}{0.08\textwidth}  
...
\label{sec:uci}
%
We consider the same 8 UCI~\cite{uci} classification benchamrks tasks and setup as were considered in \cite{add}. We use a two-layer MLP for all tasks and follow the setup in \cref{sec:methods} for \our. After training, we construct the \our dual and use the resulting model for uncertainty quantification. We performed hyperparameter search over the prior precision $\delta$, and ran the experiment over $10$ seeds. %Depending on task dimension and complexity, our model can match the neural network MAP in negative log-predictive density (NLPD) even with a low number of inducing points (see \cref{tbl:uci} left).
In \cref{tbl:uci} (left), we compare our model to the NN MAP estimate, mean-field VI, a Baysian NN, and a GLM model (see \cite{add} for the baselines) in negative log-predictive density (NLPD). We match the predictive performance despite being sparse (here, $M=$). Results for $M=16,32,64,128,256$ included in \cref{app:uci} with full experiment details. 

In \cref{tbl:uci} (right), we also include an ablation and comparison (to \cite{add}) study with a fixed low number of inducing points ($M=$). The comparison shows that our sparse method is able to summarize the information of the full data set more efficiently than sparse subset methods.
