...
In \citet{csato2002sparse}, they iteratively find dual variables setting them to log the expectation of the approximate posterior.\todo{what does it mean?} 
...
\end{equation}
where $\MKzz$ and $\vkzs$ are defined similarly to $\MKxx$ and $\vk_{\vx i}$ but over the inducing points $\MZ \in \R^{M \times D} $  instead of the full dataset $\dataset$. The key quantities we need to make predictions from our sparse GP from the converged neural network are $(\hat{\alpha}_i, \hat{\beta}_i)$\todo{these variables need another subscript for differentiate that from the values computed at the data points} and a kernel function $\kappa$. Contrasting \cref{eq:dual_sparse_post} and \cref{eq:gp_pred}, we can see that the computational complexity went from $\mathcal{O}(n^3)$  $\mathcal{O}(m^3)$, with $m \ll n$.  Crucially, given the structure of our probabilistic model, our sparse dual variables \cref{eq:dual_sparse} are a compact form \todo{use representation instead of form here ?} of the full model projected using the kernel. 

In contrast to our approach, \citet{immer2021improving} use a subset of the full GP model \todo{isn't the subset on the data points?} to reduce the computational complexity. As a result, this is a less principled approach for building a sparse model since it ignores the dataset's information. It is important to highlight that the sparsifying process is independent of the approximate inference technique because the dual variables of \cref{eq:dual_param} are computed using the neural network definition \cref{eq-empirical-risk}. 
